...
@interface NrActionSheetCell : UITableViewCell
...
@implementation NrActionSheetCell
...
@interface NrActionSheet () <UITableViewDelegate, UITableViewDataSource>
...
+ (void)actionSheetWithMessage:(NSString *)message actionArray:(NSArray<NSString *>*)actionArray cancelTitle:(NSString *)cancelTitle clickIndex:(void(^)(NSInteger index))clickIndexBlock {
    NrActionSheet *actionSheet = [[UIApplication sharedApplication].keyWindow viewWithTag:AUActionSheetTag];
    if (!actionSheet || ![actionSheet isDescendantOfView:[UIApplication sharedApplication].keyWindow]) {
        actionSheet = [[NrActionSheet alloc] initWithMessage:message destructive:NO actionArray:actionArray cancelTitle:cancelTitle clickIndex:clickIndexBlock];
...
+ (void)actionSheetDestructiveWithMessage:(NSString *)message actionArray:(NSArray<NSString *>*)actionArray cancelTitle:(NSString *)cancelTitle clickIndex:(void(^)(NSInteger index))clickIndexBlock {
    NrActionSheet *actionSheet = [[UIApplication sharedApplication].keyWindow viewWithTag:AUActionSheetTag];
    if (!actionSheet || ![actionSheet isDescendantOfView:[UIApplication sharedApplication].keyWindow]) {
        actionSheet = [[NrActionSheet alloc] initWithMessage:message destructive:YES actionArray:actionArray cancelTitle:cancelTitle clickIndex:clickIndexBlock];
        [[UIApplication sharedApplication].keyWindow addSubview:actionSheet];
    }
}
...
- (void)initSubviews {
    ...
    [self.tableView registerClass:[NrActionSheetCell class] forCellReuseIdentifier:kActionCellIdentifier];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NrActionSheetCell *cell = [tableView dequeueReusableCellWithIdentifier:kActionCellIdentifier forIndexPath:indexPath];
...
- (void)addAnimation {
    [self.contentView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(kScreenHeight-self.contentView.frame.size.height);
    }];
