...
@interface NrViewController ()<UIGestureRecognizerDelegate>
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!self.topItem){ //设置默认
        NrNavItem *item = [[NrNavItem alloc] initWithTitle:self.title];
    ...
    if (self.navShowBack && self.navigationController) {
        ...
        if ([array count] > 0) {
            ...
            if (self != rootVC && self == topVC && !self.appBar.leftBarButtonItem) {
                self.appBar.leftBarButtonItem = [[NrBarButtonItem alloc] initBackWithTarget:self action:@selector(backToPrev)];
            }
        }
    }
}
...
- (void)setTopItem:(NrNavItem *)topItem {
    _topItem = topItem;
...
- (NrNavigationBar *)appBar {
    if (!_appBar) {
        _appBar = [[NrNavigationBar alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kNaviBarHeight)];
...
- (UIView *)nrView {
    if (!_nrView) {
        ...
        _nrView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    }
    return _nrView;
}
