...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [_barButton setFrame:NrFrameAllInset(frame)];
...
- (id)initWithItem:(NrBarButtonItem *)nItem{
...
-(void)setItem:(NrBarButtonItem *)item{
    _item =item;
    [self setNeedsLayout];
}
...
-(void)layoutSubviews{
    ...
    if (self.item) {
         ...
         *使用边框填充视图
         */
        [_barButton setFrame:NrFrameInset(frame, self.inset)];
        //自定视图按钮
        if (self.item.style == NrBarButtonItemStyleCustom) {
            if (self.item.customView) {
                ...
                /*填充矩形框*/
                [self.item.customView setFrame:NrFrameInset(frame, self.inset)];
            }
        }
        //普通按钮
        else if (self.item.style == NrBarButtonItemStyleBordered) {
            if (self.item.customView) {
                if ([self.item.customView isDescendantOfView:self]) {
        ...
        //返回按钮
        else if (self.item.style == NrBarButtonItemStyleBack) {
            if (self.item.customView) {
                if ([self.item.customView isDescendantOfView:self]) {
                    [self.item.customView removeFromSuperview];
                }
