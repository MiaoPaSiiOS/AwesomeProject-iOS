...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchBlock) {
        _touchBlock(self, NrGestureRecognizerStateBegan, touches, event);
    }
    if (_longPressBlock) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchBlock) {
        _touchBlock(self, NrGestureRecognizerStateMoved, touches, event);
    }
    [self endTimer];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    if (_longPressDetected) return;
    if (_touchBlock) {
        _touchBlock(self, NrGestureRecognizerStateEnded, touches, event);
    }
    [self endTimer];
}
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event {
    if (_longPressDetected) return;
    if (_touchBlock) {
        _touchBlock(self, NrGestureRecognizerStateCancelled, touches, event);
    }
    [self endTimer];
}
