...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        queue.maxConcurrentOperationCount = 1;//maxConcurrentOperationCount不是控制线程最大数而是控制队列中同一时间片能执行多少个类型的任务操作
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(downloadMaxConcurrentCountChange:)
                                                     name:HWDownloadMaxConcurrentCountChangeNotification object:nil];
        // 是否允许蜂窝网络下载改变通知
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(downloadAllowsCellularAccessChange:)
                                                     name:HWDownloadAllowsCellularAccessChangeNotification object:nil];
        // 网路改变通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(networkingReachabilityDidChange:)
                                                     name:HWNetworkingReachabilityDidChangeNotification object:nil];
    }
