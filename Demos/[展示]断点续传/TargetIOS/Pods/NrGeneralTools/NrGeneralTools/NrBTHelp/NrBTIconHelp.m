...
#import "NrBTIconHelp.h"
#import <NrAuthorityKit/NrAuthorityKit.h>

@interface NrBTIconHelp()<UIActionSheetDelegate,UIImagePickerControllerDelegate>
...
-(void)go{
    __weak NrBTIconHelp * weakSelf=self;
...
- (void)actionClick:(NSInteger)index{
    if (index == 0) {
        NrMediaStatus status = [NrMediaManger authorizationStatusForMediaType:AVMediaTypeVideo];
        if (status != NrMediaStatusAuthorized) {
            [NrMediaManger requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
    ...
    if (index == 1) {
        NrPhotoStatus status = [NrPhotoManger authorizationStatus];
        if (status != NrPhotoStatusAuthorized) {
            [NrPhotoManger requestAuthorization:^(NrPhotoStatus status) {
                [self actionClick:index];
            }];
            return;
        }
