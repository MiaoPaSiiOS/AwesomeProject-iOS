...
@interface NrBTFileHelp : NSObject
...
/*
 ...
 + (void)achieveJSONCacheDataWithJsonUrl:(NSString *)jsonUrl
     ...
     [NrBTFileHelp asyncFetchFileAsString:filePath complete:^(id  _Nullable content) {
         if (!content || ![content isKindOfClass:[NSString class]]) {
             if (completeHandler) {
                 completeHandler(NO, nil);
             }
             // 删除缓存数据
             [NrBTFileHelp asyncRemoveItemAtPath:filePath complete:nil];
             return;
         }
         ...
         if (response.success) {
             if (response.responseObject && [response.responseObject isKindOfClass:[NSDictionary class]]) {
                 ...
                 if ([encoding isEqualToString:@"base64"]) {
                     ...
                     if (dictionary) {
                         if (![NrBTFileHelp isExistsAtPath:filePath]) {
                             [NrBTFileHelp createFileAtPath:filePath];
                         }
                         [NrBTFileHelp asyncWriteFileAtPath:filePath content:[self JSONString:dictionary] complete:nil];
                         if (completeHandler) {
                             completeHandler(YES, dictionary);
                         }
                     } else {
 ...
 + (NSString *)cacheJSONDataFoldPath {
     if (![NrBTFileHelp isExistsAtPath:[[NrBTFileHelp documentsDir] stringByAppendingPathComponent:kUIDataPageFold]]) {
         [NrBTFileHelp createDirectoryAtPath:[[NrBTFileHelp documentsDir] stringByAppendingPathComponent:kUIDataPageFold]];
     }
     return [[NrBTFileHelp documentsDir] stringByAppendingPathComponent:kUIDataPageFold];
 }
