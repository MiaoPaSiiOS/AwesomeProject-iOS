...
+ (NrFile *)scanRelativeFilePath:(NSString *)relativeFilePath maxTreeLevel:(NSInteger)maxTreeLevel {
    
    NrFile *file = nil;
    
    // Get the real file path.
    NSString *filePath = [NrFileManager absoluteFilePathFromRelativeFilePath:relativeFilePath];
    ...
    if (isExist) {
        ...
        file = [NrFileManager cerateFileWithFilePath:filePath isDirectory:isDirectory];
        
        if (file.isDirectory) {
            
            [NrFileManager scanDir:file.filePath rootFile:file maxScanLevel:(maxTreeLevel <= 0 ? 0 : maxTreeLevel)];
        }
    }
...
+ (NrFile *)cerateFileWithFilePath:(NSString *)filePath isDirectory:(BOOL)isDirectory {
    
    NrFile *file        = [[NrFile alloc] init];
...
+ (void)scanDir:(NSString *)dirPath rootFile:(NrFile *)rootFile maxScanLevel:(NSInteger)maxLevel {
    ...
    for (NSString *relatedPath in array) {
        ...
        NrFile *file = [NrFileManager cerateFileWithFilePath:fullPath isDirectory:isDirectory];
        file.level = rootFile.level + 1;
        
        if (file.isDirectory) {
            
            [NrFileManager scanDir:file.filePath rootFile:file maxScanLevel:maxLevel];
        }
