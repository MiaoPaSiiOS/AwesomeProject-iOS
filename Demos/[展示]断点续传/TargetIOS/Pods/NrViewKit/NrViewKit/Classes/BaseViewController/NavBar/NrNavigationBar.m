...
@interface NrNavigationBar ()
@property (nonatomic, strong) NrDataView *topView;
@property (nonatomic, strong) NrBarButton *rightBarButton;
@property (nonatomic, strong) UIImageView *topImageView;
@end

@implementation NrNavigationBar
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _leftBarButton = [[NrBarButton alloc] initWithFrame:CGRectZero];
        [self addSubview:_leftBarButton];
        
        _rightBarButton = [[NrBarButton alloc] initWithFrame:CGRectZero];
        [self addSubview:_rightBarButton];
        
        _topView = [[NrDataView alloc] initWithFrame:CGRectZero];
...
- (void)layoutSubviews {
    ...
    if (self.leftBarButtonItem) {
        ...
        if (_leftBarButtonItem.style == NrBarButtonItemStyleBack) {
    ...
    [_topView setFrame:NrFrameAll(0,y,w,h)];
    [_topView setCenterX:self.navigationBar.centerX];
   
    [_topTitleView setFrame:NrFrameInset(_topView.frame,self.inset)];
    [_topImageView setFrame:NrFrameInset(_topView.frame,self.inset)];
    //  设置标题视图
    if (self.topItem) {
...
- (CGSize)barButtonSize:(NrBarButtonItem *)item {
    ...
    else if (item.style == NrBarButtonItemStyleCustom) {
    ...
    else if (item.style == NrBarButtonItemStyleBack) {
...
- (void)setTopItem:(NrNavItem *)nTopItem {
    _topItem = nTopItem;
    [self setNeedsLayout];
}

- (void)setLeftBarButtonItem:(NrBarButtonItem *)leftBarButtonItem {
    _leftBarButtonItem = leftBarButtonItem;
    [self setNeedsLayout];
}

- (void)setRightBarButtonItem:(NrBarButtonItem *)rightBarButtonItem{
