...
typedef NS_ENUM(NSInteger, NrContactStatus) {
    /// 表明用户尚未选择关于客户端是否可以访问通讯录
    NrContactStatusNotDetermined = 0,
    /// 客户端未被授权访问通讯录。用户不能改变客户机的状态,可能由于活跃的限制,如家长控制
    NrContactStatusRestricted,
    /// 明确拒绝用户访问通讯录
    NrContactStatusDenied,
    /// 用户授权访问通讯录
    NrContactStatusAuthorized
};

NS_ASSUME_NONNULL_BEGIN

@interface NrAddressBookManger : NSObject
