...
+ (NSString *)nr_deviceModel {
    if ([self nr_isSimulator]) {
...
+ (NSString *)nr_deviceName {
    static dispatch_once_t onceToken;
    static NSString *name;
    dispatch_once(&onceToken, ^{
        NSString *model = [self nr_deviceModel];
        if (!model) {
            name = @"Unknown Device";
            return;
        }
        ...
        name = dict[model];
        if (!name) name = model;
        if ([self nr_isSimulator]) name = [name stringByAppendingString:@" Simulator"];
    });
    return name;
}
