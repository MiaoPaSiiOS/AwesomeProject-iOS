                            ...
                            parseError:(NSError *)parseError
{
    ...
    if (self) {
        ...
        if (parseError) {
            ...
            if ([DSHelper isDictEmptyOrNil:(responseObject)]) {
                self.success    = NO;
            } else {
                NSString *code = [DSHelper safeString:(responseObject[@"code"])];
...
- (NSString *)message {
    ...
    if (self.error) {
        ...
        return [NSString stringWithFormat:@"%@",[DSHelper safeString:(self.responseObject[@"message"])]];
    }
    return nil;
}
