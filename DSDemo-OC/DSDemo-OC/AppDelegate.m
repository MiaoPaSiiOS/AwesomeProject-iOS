...
- (void)goToTabWithIndex:(NSNumber*)idx {
    ...
    if ([REGlobal sharedInstance].globalTbc.selectedIndex == index) {
        [[DSHelper findTopViewController].navigationController popToRootViewControllerAnimated:YES];
    } else {
        //如果是在一级页面，则直接跳转到index；非一级页面，给个延迟再跳转
        if (![DSHelper findTopViewController].tabBarController.tabBar.hidden) {
