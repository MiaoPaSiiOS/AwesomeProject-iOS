...
+ (instancetype)sharedInstance {
    ...
    static DSDeviceInfo *instance = nil;
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.deviceModel = [DSDeviceInfo deviceModel];
        self.ostype = [UIDevice currentDevice].systemName;
        self.deviceName = [UIDevice currentDevice].name;
        self.deviceid =  [UIDevice currentDevice].identifierForVendor.UUIDString;
        self.deviceid = [self.deviceid stringByReplacingOccurrencesOfString:@"-" withString:@""];
        self.deviceid = [self.deviceid substringToIndex:16];
        
        // iOS14兼容, IDFA 权限判断, IDFA只在长尾客户消费信贷需求中使用, 该需求目前已下线
        if (@available(iOS 14, *)) {
        ...
        self.appID = [[[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleIdentifier"] pathExtension];
