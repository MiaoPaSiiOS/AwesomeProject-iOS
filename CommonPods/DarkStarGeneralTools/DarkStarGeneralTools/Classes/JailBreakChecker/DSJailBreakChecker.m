...
@implementation DSJailBreakChecker
/*
 1.通过越狱后增加的越狱文件判断
...
+ (BOOL)isJailBroken {
    if ([DSJailBreakChecker isJailBreak1]){
        return YES;
    }
    if ([DSJailBreakChecker isJailBreak2]){
        return YES;
    }
    if ([DSJailBreakChecker isJailBreak3]){
        return YES;
    }
    return NO;
}
