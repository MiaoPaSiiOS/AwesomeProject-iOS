...
+ (DSFile *)scanRelativeFilePath:(NSString *)relativeFilePath maxTreeLevel:(NSInteger)maxTreeLevel {
    
    DSFile *file = nil;
    
    // Get the real file path.
    NSString *filePath = [DSFileManager absoluteFilePathFromRelativeFilePath:relativeFilePath];
    ...
    if (isExist) {
        ...
        file = [DSFileManager cerateFileWithFilePath:filePath isDirectory:isDirectory];
        
        if (file.isDirectory) {
            
            [DSFileManager scanDir:file.filePath rootFile:file maxScanLevel:(maxTreeLevel <= 0 ? 0 : maxTreeLevel)];
        }
    }
...
+ (DSFile *)cerateFileWithFilePath:(NSString *)filePath isDirectory:(BOOL)isDirectory {
    
    DSFile *file        = [[DSFile alloc] init];
...
+ (void)scanDir:(NSString *)dirPath rootFile:(DSFile *)rootFile maxScanLevel:(NSInteger)maxLevel {
    ...
    for (NSString *relatedPath in array) {
        ...
        DSFile *file = [DSFileManager cerateFileWithFilePath:fullPath isDirectory:isDirectory];
        file.level = rootFile.level + 1;
        
        if (file.isDirectory) {
            
            [DSFileManager scanDir:file.filePath rootFile:file maxScanLevel:maxLevel];
        }
