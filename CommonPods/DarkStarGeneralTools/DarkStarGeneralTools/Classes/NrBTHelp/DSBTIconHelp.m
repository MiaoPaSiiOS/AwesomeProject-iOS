...
#import "DSBTIconHelp.h"
#import "DarkStarAuthorityKit.h"

@interface DSBTIconHelp()<UIActionSheetDelegate,UIImagePickerControllerDelegate>
...
-(void)go{
    __weak DSBTIconHelp * weakSelf=self;
...
- (void)actionClick:(NSInteger)index{
    if (index == 0) {
        DSMediaStatus status = [DSMediaManger authorizationStatusForMediaType:AVMediaTypeVideo];
        if (status != DSMediaStatusAuthorized) {
            [DSMediaManger requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
    ...
    if (index == 1) {
        DSPhotoStatus status = [DSPhotoManger authorizationStatus];
        if (status != DSPhotoStatusAuthorized) {
            [DSPhotoManger requestAuthorization:^(DSPhotoStatus status) {
                [self actionClick:index];
            }];
            return;
        }
