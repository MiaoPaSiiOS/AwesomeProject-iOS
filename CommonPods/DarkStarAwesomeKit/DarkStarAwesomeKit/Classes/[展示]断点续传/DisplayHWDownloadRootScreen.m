...
- (void)projectOnceCode
{
    ...
    if (![defaults boolForKey:onceKey]) {
        ...
        [defaults setInteger:10 forKey:HWDownloadMaxConcurrentCountKey];
...
- (void)viewDidLoad {
    ...
    [self.dsView addSubview:self.myCategoryView];
    
    
    self.listContainerView = [[JXCategoryListContainerView alloc] initWithType:JXCategoryListContainerType_ScrollView delegate:self];
    self.listContainerView.scrollView.scrollEnabled = NO;
    [self.dsView addSubview:self.listContainerView];
    // 关联到 categoryView
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat totalItemWidth = self.dsView.width - 30*2;
    self.myCategoryView.frame = CGRectMake(30, 10, totalItemWidth, 30);
    self.listContainerView.frame = CGRectMake(0, 50, self.dsView.width, self.dsView.height - 50);
}

- (void)viewDidAppear:(BOOL)animated {
    [super viewDidAppear:animated];
    DSBarButtonItem *rightBarButtonItem = [[DSBarButtonItem alloc] initWithTitle:@"设置" target:self action:@selector(showDisplayOfCommonFunctions)];
