...
function srsCallNative(nativeMethodName, parameters, htmlCallBack) {
  ...
  if (navigator.userAgent.indexOf("com.bankcomm.app.mdb") != -1) {
    if (window.metaObj) {
      ...
      var result = {
        ...
        srsCallback: "srsCallback",
      };
      var result = window.metaObj.call(JSON.stringify(result));
    } else {
    ...
    // 安卓
    var parametersStr = JSON.stringify(parameters);
    if (window.metaObj && typeof window.metaObj.srsNativeMethod == "function") {
      window.metaObj.srsNativeMethod(
        nativeMethodName,
        parametersStr,
        htmlCallBack,
        "srsCallback"
...
function srsCheckNativeMethodExist(nativeMethodName, nativeMethodExist) {
  ...
  if (navigator.userAgent.indexOf("com.bankcomm.app.mdb") != -1) {
    if (window.metaObj) {
      var result = {
        method: "JSCheckMethodExist",
        parameters: nativeMethodName,
        htmlCallBack: nativeMethodExist,
        srsCallback: "srsCallback",
      };
      var result = window.metaObj.call(JSON.stringify(result));
    } else {
    ...
    if (
      window.metaObj &&
      typeof window.metaObj.srsCheckMethodExist == "function"
    ) {
      window.metaObj.srsCheckMethodExist(nativeMethodName, nativeMethodExist);
    } else {
      window[nativeMethodExist] &&
        typeof window[nativeMethodExist] === "function" &&
        window[nativeMethodExist](nativeMethodName, "false");
    }
  } else {
    //苹果
    var nativePara = {};
...
function srsParametersTypeError(nativeMethodName, errorInfo) {
  console.log(
    "Method of " + nativeMethodName + " parameters type error," + errorInfo
  );
}
