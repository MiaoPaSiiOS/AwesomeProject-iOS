...
- (instancetype)init
{
    ...
    if (self) {
        ...
        if (![DSHelper isArrayEmptyOrNil:(PAGEWHITELIST)]) {
        ...
        if (![DSHelper isArrayEmptyOrNil:(WEBCACHEWHITELIST)]) {
...
+ (BOOL)SpecialIOSListFlag:(NSString *)netUrl {
    if ([DSHelper isStringEmptyOrNil:(netUrl)]) {
        return YES;
    }
...
//    [netPageUrl addObject:dictDomain5];
    ...
    for (NSDictionary *domain in netPageUrl) {
        ...
        if ([DSHelper isStringEmptyOrNil:(host)]) {
            return NO;
        }
...
+ (BOOL)parentString:(NSString *)pString containsString:(NSString *)aString {
    ...
    if (!DSDeviceInfo.isIOS8Later) {
        if(![DSHelper isStringEmptyOrNil:(pString)] && [pString rangeOfString:aString].location != NSNotFound) {
            return YES;
        }
    }else{
