...
@implementation AmenJsbWKWebView
- (instancetype)initWithFrame:(CGRect)frame configuration:(WKWebViewConfiguration *)configuration {
    ...
    configuration.processPool = [AmenJsbWKWebView processPool];
...
- (WKNavigation *)loadRequest:(NSURLRequest *)request {
    NSMutableURLRequest *muRequest = [AMENWebViewCacheManager achiveRequestWithRequest:request];
    [AmenJsbWKWebView setUserAgentWithWKWebView:self];
...
- (void)syncAjaxCookie {
    [self evaluateJavaScript:[AmenJsbWKWebViewCookieManager ajaxCookieScripts] completionHandler:^(id _Nullable content, NSError * _Nullable error) {
    }];
}
