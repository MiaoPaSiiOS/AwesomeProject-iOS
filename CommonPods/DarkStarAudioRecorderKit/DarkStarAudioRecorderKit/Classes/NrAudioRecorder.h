...
typedef NS_ENUM(NSUInteger, NrAudioRecorderStatus) {
    NrAudioRecorderStatusInitFailure,      /// 初始化失败
    NrAudioRecorderStatusNoApplyAuthority, /// 未申请麦克风权限
    NrAudioRecorderStatusNoAuthorized,     /// 无麦克风权限
    NrAudioRecorderStatusSuccess,          /// 录音成功
    NrAudioRecorderStatusFailure           /// 录音失败
};

typedef void (^AudioRecordCompleteHandler) (NrAudioRecorderStatus status, NSData * _Nullable data);
typedef void (^RecorderPower) (float power);

@interface NrAudioRecorder : NSObject
