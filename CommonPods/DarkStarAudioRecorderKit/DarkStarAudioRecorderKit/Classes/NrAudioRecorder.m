...
@interface NrAudioRecorder ()<AVAudioRecorderDelegate>
...
@implementation NrAudioRecorder
...
+ (instancetype)audioRecorderWithCompleteHandler:(AudioRecordCompleteHandler)completeHandler {
    NrAudioRecorder *recorder = [[self alloc] initWithCompleteHandler:completeHandler];
    return recorder;
}
...
+ (instancetype)audioRecordWithSettings:(NSDictionary<NSString *,id> *)settings completeHandler:(AudioRecordCompleteHandler)completeHandler {
    NrAudioRecorder *recorder = [[self alloc] initWithSettings:settings completeHandler:completeHandler];
    return recorder;
}
...
- (void)startRecorder {
    ...
    if (self.settings.allKeys.count == 0) {
        self.completeHandler(NrAudioRecorderStatusInitFailure, nil);
        return;
    }
    ...
    if (status == DSMediaStatusNotDetermined) {
        // 未申请过麦克风权限
        self.completeHandler(NrAudioRecorderStatusNoApplyAuthority, nil);
        return;
    } else if (status == DSMediaStatusRestricted || status == DSMediaStatusDenied) {
        // 无麦克风权限
        self.completeHandler(NrAudioRecorderStatusNoAuthorized, nil);
        return;
    }
    ...
    if (!self.recorder || error) {
        self.completeHandler(NrAudioRecorderStatusFailure, nil);
        self.recorder = nil;
        return;
    }
...
- (void)stopRecorder {
    if (self.recorder && self.recorder.isRecording) {
        ...
        if (self.completeHandler) {
            ...
            if (len > 0) {
                self.completeHandler(NrAudioRecorderStatusSuccess, data);
            } else {
                self.completeHandler(NrAudioRecorderStatusFailure, nil);
            }
        }
