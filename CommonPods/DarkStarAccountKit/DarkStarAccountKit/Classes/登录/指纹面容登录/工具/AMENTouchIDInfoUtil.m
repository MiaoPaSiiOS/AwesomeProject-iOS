...
@implementation AMENTouchIDInfoUtil
#pragma mark - 验证设备是否支持TouchID或者FaceID
+ (BOOL)returnTheResultOfPermissions {
    if ([self judueIPhonePlatformSupportTouchID:9] && isIOS11Later) {//iPhone8、8P、X 及以后手机  iOS11及以后系统
        ...
        if ([self judueIPhonePlatformSupportTouchID:5] && isIOS8Later) {//5s及以后手机  iOS8及以后系统
            LAContext *context = [[LAContext alloc] init];
            NSError *error = nil;
...
+ (BOOL)returnTheResultOfFaceID {
    if ([self judueIPhonePlatformSupportTouchID:9] && isIOS11Later) {
        LAContext *context = [[LAContext alloc] init];
        NSError *error = nil;
...
+ (BOOL)returnStateOfSettingButton {
    LAContext *context = [[LAContext alloc] init];
    NSError *error = nil;
    if ([self judueIPhonePlatformSupportTouchID:9] && isIOS11Later && [self returnTheResultOfFaceID]) {
        // LAPolicyDeviceOwnerAuthenticationWithBiometrics: 用TouchID/FaceID验证
        // LAPolicyDeviceOwnerAuthentication: 用TouchID/FaceID或密码验证, 默认是错误两次或锁定后, 弹出输入密码界面（本案例使用）
