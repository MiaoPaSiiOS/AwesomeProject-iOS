...
@implementation AMENVocalPrintInfoUtil
+ (void)vocalOpenStatusDes:(VocalStatus)vStatus leave:(LeaveStatus)leave {

    //声纹开关（1打开  0关闭）
    NSString *vocalPrintSwitchStr = safeString([[NSUserDefaults standardUserDefaults] objectForKey:@"VocalPrintSwitch"]);
    ...
    if ([verifyLevelStr isEqualToString:@"1"] || [verifyLevelStr isEqualToString:@"2"]) {
        NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:safeString([DSUserSession sharedInstance].userid),@"userId",nil];
        // 查询声纹是否开通
        [[DSNetWorkManager sharedInstance] dataTaskWithUrlPath:@"" requestType:DSNetworkRequestTypeGET header:nil params:dict completionHandler:^(DSNetResponse *response) {
            if (response.success) {
                [[NSUserDefaults standardUserDefaults] setObject:safeString(response.responseObject[@"vocalToken"]) forKey:@"vocalToken"];
                [[NSUserDefaults standardUserDefaults] setObject:safeString(response.responseObject[@"vocalText"]) forKey:@"vocalText"];
                [[NSUserDefaults standardUserDefaults] synchronize];
            } else {
                [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"vocalText"];
                [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"vocalToken"];
