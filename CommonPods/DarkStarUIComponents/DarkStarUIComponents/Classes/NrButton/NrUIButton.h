...
/// 控制图片在UIButton里的位置，默认为NrUIButtonImagePositionLeft
typedef NS_ENUM(NSUInteger, NrUIButtonImagePosition) {
    NrUIButtonImagePositionTop,             // imageView在titleLabel上面
    NrUIButtonImagePositionLeft,            // imageView在titleLabel左边
    NrUIButtonImagePositionBottom,          // imageView在titleLabel下面
    NrUIButtonImagePositionRight,           // imageView在titleLabel右边
};
 ...
 *  @warning NrUIButton 重新定义了 UIButton.titleEdgeInests、imageEdgeInsets、contentEdgeInsets 这三者的布局逻辑，sizeThatFits: 里会把 titleEdgeInests 和 imageEdgeInsets 也考虑在内（UIButton 不会），以使这三个接口的使用更符合直觉。
 */
@interface NrUIButton : UIButton
...
/**
 * 设置按钮里图标和文字的相对位置，默认为NrUIButtonImagePositionLeft<br/>
 * 可配合imageEdgeInsets、titleEdgeInsets、contentHorizontalAlignment、contentVerticalAlignment使用
 */
@property(nonatomic, assign) NrUIButtonImagePosition imagePosition;
