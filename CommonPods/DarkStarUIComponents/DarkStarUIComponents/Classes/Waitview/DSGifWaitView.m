...
-(id)initWithSuperViewController:(UIViewController *)viewController style:(PNCGifWaitingStyle)style
{
    if (self = [super init]) {
        ...
        self.size = CGSizeMake([DSCommonMethods screenWidth], [DSCommonMethods screenHeight] - kNaviBarHeight);
...
- (void)drawFrame:(PNCGifWaitingStyle)style {
    ...
    if (style == DefaultWaitStyle || style == LongLoginWaitStyle) { //灰底白色转圈样式
        ...
        if(self.height == [DSCommonMethods screenHeight] - kNaviBarHeight) {
            mainview.origin = CGPointMake((self.width - (75))/2, [DSCommonMethods screenHeight]*0.4 - (75)/2 - kNaviBarHeight);
        }
        if (self.isRotate) {
            //旋转90度
        ...
        self.WaitImgView.animationImages = [DSCacheManger sharedInstance].refreshWhiteLoadingArray;
        ...
        if(style != PartLoadWaitStyle && self.height == [DSCommonMethods screenHeight] - kNaviBarHeight) {
            mainview.origin = CGPointMake((self.width - (55))/2, [DSCommonMethods screenHeight]*0.4 - (55)/2 - kNaviBarHeight);
        }
        if (self.isRotate) {
            //旋转90度
            mainview.transform = CGAffineTransformRotate(mainview.transform, M_PI*0.5);
        ...
        self.WaitImgView.animationImages = [DSCacheManger sharedInstance].refreshBlueLoadingArray;
        self.WaitImgView.animationDuration = 1;
        self.WaitImgView.animationRepeatCount = 0;
        [self.WaitImgView startAnimating];
