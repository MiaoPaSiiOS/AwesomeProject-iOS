...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    return [DSHelper isArrayEmptyOrNil:(self.showDatas)] ? 0 : self.showDatas.count;
}

#pragma mark - UIPickerViewDelegate
// returns width of column and height of row for each component.
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component {
    return DSDeviceInfo.screenWidth - 20;
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (customView == nil) {
        customView = [[DSSingleChoiceDialogReusingView alloc] initWithFrame:CGRectMake(0, 0,DSDeviceInfo.screenWidth - 20,44.f)];
    }
