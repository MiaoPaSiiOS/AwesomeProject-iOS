...
static CGFloat const kViewHeight = 216;
static CGFloat const kToolbarHeight = 37;
static CGFloat const kOneEmoticonHeight = 50;
static NSInteger const kOnePageCount = 20;
...
- (void)startBackspaceTimer {
    ...
    _backspaceTimer = [NSTimer timerWithTimeInterval:0.1 block:^(NSTimer *timer) {
        ...
        if (cell.isDelete) {
            if ([strongSelf.delegate respondsToSelector:@selector(emoticonScrollViewDidTapCell:)]) {
                [[UIDevice currentDevice] playInputClick];
                [((id<WBEmoticonScrollViewDelegate>) strongSelf.delegate) emoticonScrollViewDidTapCell:cell];
            }
        }
    } repeats:YES];
...
- (instancetype)init {
    ...
    self.frame = CGRectMake(0, 0, DSDeviceInfo.screenWidth, kViewHeight + SafeAreaInsetsConstantForDeviceWithNotch.bottom);
...
- (void)_initCollectionView {
    CGFloat itemWidth = (DSDeviceInfo.screenWidth - 10 * 2) / 7.0;
    itemWidth = [DSComputer CGFloatPixelRound:(itemWidth)];
    CGFloat padding = (DSDeviceInfo.screenWidth - 7 * itemWidth) / 2.0;
    CGFloat paddingLeft = [DSComputer CGFloatPixelRound:(padding)];
    CGFloat paddingRight = DSDeviceInfo.screenWidth - paddingLeft - itemWidth * 7;
    ...
    _collectionView = [[WBEmoticonScrollView alloc] initWithFrame:CGRectMake(0, 0, DSDeviceInfo.screenWidth, kOneEmoticonHeight * 3) collectionViewLayout:layout];
    ...
    _pageControl.size = CGSizeMake(DSDeviceInfo.screenWidth, 20);
...
- (void)_initToolbar {
    ...
    toolbar.size = CGSizeMake(DSDeviceInfo.screenWidth, kToolbarHeight);
    ...
    for (NSUInteger i = 0; i < _emoticonGroups.count; i++) {
        ...
        btn.left = DSDeviceInfo.screenWidth / (float)_emoticonGroups.count * i;
        btn.tag = i;
        [scroll addSubview:btn];
        [btns addObject:btn];
    }
    
    toolbar.bottom = self.container.height;
