...
- (void)_layoutTag {
    ...
    if (tag.tagType == 1) {
        ...
        [text yy_setColor:kWBCellTextHighlightColor range:text.yy_rangeOfAll];
...
- (NSMutableAttributedString *)_textWithStatus:(WBStatus *)status
        ...
        do {
            ...
            if ([text yy_attribute:YYTextHighlightAttributeName atIndex:range.location] == nil) {
                ...
                if (wburl.urlTypePic.length) {
                    ...
                    UIImage *image = [[SDImageCache sharedImageCache] imageFromCacheForKey:picURL.absoluteString];
    ...
    // 匹配 @用户名
    NSArray *atResults = [[WBStatusHelper regexAt] matchesInString:text.string options:kNilOptions range:text.yy_rangeOfAll];
