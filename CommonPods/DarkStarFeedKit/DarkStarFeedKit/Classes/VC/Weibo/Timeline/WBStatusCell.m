...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    /*
     默认是NO，如果设置为YES则当前UIView会独占整个Touch事件。
     具体来说就是如果UIView设置了exclusiveTouch属性为YES则当这个UIView成为第一响应者时，
     在手指离开屏幕前其他view不会响应任何touch事件。
     */
    ...
    switch (isRetweet ? layout.retweetCardType : layout.cardType) {
        ...
        case WBStatusCardTypeNormal: {
            ...
            if (button.pic && button.name) {
                ...
                [_button sd_setImageWithURL:button.pic forState:UIControlStateNormal placeholderImage:nil];
            } else {
                _button.hidden = YES;
            }
        }break;
        case WBStatusCardTypeVideo: {
            ...
            [_imageView sd_setImageWithURL:pageInfo.pagePic];
            _button.hidden = NO;
            _button.frame = self.bounds;
            [_button setTitle:nil forState:UIControlStateNormal];
            [_button sd_cancelImageLoadForState:UIControlStateNormal];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _bottomLine.backgroundColor = [DSHelper colorWithHexString:@"0xE8E8E8"].CGColor;
    ...
    [_repostButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        kStrongSelf
        WBStatusCell *cell = strongSelf.cell;
...
- (void)setLiked:(BOOL)liked withAnimation:(BOOL)animation {
    ...
    likeText.yy_color = liked ? kWBCellToolbarTitleHighlightColor : kWBCellToolbarTitleColor;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    [_button addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        ...
        if ([strongSelf.cell.delegate respondsToSelector:@selector(cellDidClickTag:)]) {
            [strongSelf.cell.delegate cellDidClickTag:strongSelf.cell];
        }
    }];
    _button.hidden = YES;
    [self addSubview:_button];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    [_menuButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
        kStrongSelf
        if ([strongSelf.cell.delegate respondsToSelector:@selector(cellDidClickMenu:)]) {
            [strongSelf.cell.delegate cellDidClickMenu:strongSelf.cell];
        }
    }];
        ...
        imageView.touchBlock = ^(DSControl *view, DSGestureRecognizerState state, NSSet *touches, UIEvent *event) {
            ...
            if (state == DSGestureRecognizerStateEnded) {
                ...
                if (CGRectContainsPoint(view.bounds, p)) {
                    [strongSelf.cell.delegate cell:strongSelf.cell didClickImageAtIndex:i];
                }
            }
        };
...
- (void)setLayout:(WBStatusLayout *)layout {
    ...
    [_vipBackgroundView sd_setImageWithURL:picBg placeholderImage:nil options:SDWebImageAvoidAutoSetImage completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        if (image) {
            image = [UIImage imageWithCGImage:image.CGImage scale:2.0 orientation:image.imageOrientation];
            vipBackgroundView.image = image;
        }
    }];
//    _vipBackgroundView.layer.borderColor = [UIColor redColor].CGColor;
//    _vipBackgroundView.layer.borderWidth = 2;
...
- (void)_setImageViewWithTop:(CGFloat)imageTop isRetweet:(BOOL)isRetweet {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (i >= picsCount) {
            ...
            picView.frame = (CGRect){.origin = origin, .size = picSize};
            picView.hidden = NO;
            WBPicture *pic = pics[i];
            
            UIView *badge = picView.subviews.firstObject;
            ...
            [imageView sd_setImageWithURL:pic.bmiddle.url placeholderImage:nil options:SDWebImageAvoidAutoSetImage completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                ...
                if (image) {
                    ...
                    CGFloat scale = (height / width) / (strongImageView.height / strongImageView.width);
                    if (scale < 0.99 || isnan(scale)) { // 宽图把左右两边裁掉
                        strongImageView.contentMode = UIViewContentModeScaleAspectFill;
                        strongImageView.layer.contentsRect = CGRectMake(0, 0, 1, 1);
                    } else { // 高图只保留顶部
                        strongImageView.contentMode = UIViewContentModeScaleToFill;
                        strongImageView.layer.contentsRect = CGRectMake(0, 0, 1, (float)width / height);
                    }
