...
{
    NSString *path = [AmShViUtils jsonPathWithJsonNamed:@"MHFilter"];
...
+ (NSArray<MHFilterInfo *> *)readAllCommonFiltersArr
{
    ...
    NSString *path = [AmShViUtils jsonPathWithJsonNamed:@"MHVideoCommonFilter"];
    NSString *jsonString = [[NSString alloc] initWithContentsOfFile:path encoding:NSUTF8StringEncoding error:NULL];
    if (!jsonString) {
        return nil;
    }
...
+ (NSArray<MHFilterInfo *> *)readAllSpecialFiltersArr
{
    ...
    NSString *path = [AmShViUtils jsonPathWithJsonNamed:@"MHVideoSpecialFilter"];
    NSString *jsonString = [[NSString alloc] initWithContentsOfFile:path encoding:NSUTF8StringEncoding error:NULL];
    if (!jsonString) {
        return nil;
    }
