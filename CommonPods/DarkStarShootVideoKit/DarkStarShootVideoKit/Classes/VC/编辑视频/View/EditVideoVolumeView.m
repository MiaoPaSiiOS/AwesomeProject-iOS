...
{
    CGFloat contentHeight = ((48) + (145) + SafeAreaInsetsConstantForDeviceWithNotch.bottom);
    ...
    surebtn.frame = CGRectMake(self.width - 32 -15, 0, 52, 32);
    ...
    titleLab.frame = CGRectMake(0, 0, self.width, 32);
    titleLab.text = @"音量";
    [contentView addSubview:titleLab];
    
    //黑色容器
    UIView * sliderContent = [[UIView alloc] initWithFrame:CGRectMake(0, (48), self.frame.size.width, contentView.frame.size.height - (48))];
    ...
    NSArray * arr = @[@"原生",@"配乐"];
    for (int i = 0; i < 2; i ++) {
        UIView * item = [[UIView alloc] initWithFrame:CGRectMake(0, sliderContent.frame.size.height/2*i, sliderContent.frame.size.width, sliderContent.frame.size.height/2 - (10))];
