...
#import "AmenEditVideoScreen.h"

#define TIMER_INTERVAL  0.05    //定时器时间间隔
#define MAX_VIDEOTIME  15.f     //视频最大录制时长

@interface AmenShootVideoScreen ()
<
ShootRightBarDelegate,
ShootBottomBarDelegate,
FastSlowChooseDelegate,
ChooseBGMusicDelegate
>
{
    GPUImageMovieWriterEx * _movieWriter;//输出控制
}
@property(nonatomic,strong) PostVideoModel * videoModel;//保存视频信息的模型
@property(nonatomic,strong) UIImageView * imgView;//相机layer
...
@implementation AmenShootVideoScreen
...
{
    //拍摄进度bar
    self.progressBar = [[ShootProgressBar alloc] initWithFrame:CGRectMake((10), SafeAreaInsetsConstantForDeviceWithNotch.top, self.view.width - (20), (5))];
    [self.view addSubview:self.progressBar];
    
    
    //退出按钮
    self.quit_Btn = [UIButton buttonWithType:0];
    [self.quit_Btn setImage:AmShViImage(@"btn_back_white") forState:0];
    [self.quit_Btn setImage:AmShViImage(@"btn_back_white") forState:UIControlStateHighlighted];
    self.quit_Btn.frame = CGRectMake((15), CGRectGetMaxY(self.progressBar.frame)+10, (40), (40));
    ...
    //右侧按钮集合
    self.rightBar = [[ShootRightBar alloc] initWithFrame:CGRectMake(self.view.width - (50)-(15), CGRectGetMaxY(self.progressBar.frame)+10, (50), ((50) + (10))*6)];
    ...
    //顶部背景音乐bar
    self.musicBar = [[ShootMusicBar alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.quit_Btn.frame) + 10, CGRectGetMaxY(self.progressBar.frame)+10, (CGRectGetMinX(self.rightBar.frame) - CGRectGetMaxX(self.quit_Btn.frame) - 20), (40))];
    ...
    //底部按钮集合
    self.bottomBar = [[ShootBottomBar alloc] initWithFrame:CGRectMake(0, self.view.height - (150) - SafeAreaInsetsConstantForDeviceWithNotch.bottom, self.view.width, (150))];
...
{
//     MHToast *toast = [MHToast showInView:nil];
    [self re_showHUDMessage:nil];
    //开始视频变速+合并
    [PostVideoModel compositionSubVideos:self.videoModel.subVideoInfos callBack:^(BOOL success, NSString *outPurPath) {
        ...
        if (success) {
            ...
            AmenEditVideoScreen * vc = [[AmenEditVideoScreen alloc] init];
            vc.videoModel = self.videoModel;
            [self.navigationController pushViewController:vc animated:YES];
            
        }else{
            [self re_showMSG:@"频变速+合并:失败"];
            NSLog(@"最终还是失败了额");
        }
    }];
}
...
{
    NSLog(@"跳转 选择音乐");
    AmenChooseBgMusicScreen *vc = [[AmenChooseBgMusicScreen alloc] init];
    vc.delegate = self;
    [self.navigationController pushViewController:vc animated:YES];
}
#pragma mark - 音乐选择回调
...
{
    NSLog(@"选中的背景音乐 %@",choosedMusicName);
...
{
    if (!_focusView) {
        _focusView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 90, 90)];
...
{
    if (!_fastSlowBar) {
        _fastSlowBar = [[FastSlowChooseView alloc] initWithFrame:CGRectMake((35), CGRectGetMinY(self.bottomBar.frame) - 36, self.view.width - (35)*2, (36))];
