...
-(void)commit_subViews
{
    ...
    // 130 + 90
    //整体容器
    self.contentView = [[UIView alloc] initWithFrame:CGRectMake(0, self.height - (220+SafeAreaInsetsConstantForDeviceWithNotch.bottom), self.width, 220)];
    [self addSubview:self.contentView];
    
    //提示lab
    self.tipLab = [[UILabel alloc] init];
    self.tipLab.textColor = [UIColor whiteColor];
    self.tipLab.font = [UIFont boldSystemFontOfSize:14];
    self.tipLab.textAlignment = NSTextAlignmentCenter;
    self.tipLab.frame = CGRectMake(0, 0, self.contentView.frame.size.width, 32);
    self.tipLab.text = @"左右拖动声谱以剪取音乐";
    [self.contentView addSubview:self.tipLab];
    
    //确认按钮
    self.sureBtn = [UIButton buttonWithType:0];
    self.sureBtn.frame = CGRectMake(self.contentView.width - 55 - 15, 0, 55, 32);
    ...
    //当前开始时间
    self.curentTimeLab = [[UILabel alloc] init];
    self.curentTimeLab.textColor = [UIColor whiteColor];
    self.curentTimeLab.font = [UIFont systemFontOfSize:14];
    self.curentTimeLab.textAlignment = NSTextAlignmentCenter;
    self.curentTimeLab.frame = CGRectMake(20, self.contentView.height-130-25, 0, 20);
...
-(void)update_curentTimeLab
{
    ...
    CGFloat strWidth = [str ds_widthWithFont:self.curentTimeLab.font] + 16;
...
-(void)commit_scrollView
{
    ...
    //初始化scrollview
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 90, self.frame.size.width, 130)];
    ...
    CGFloat perPinPuHeight = (self.scrollView.frame.size.height - 30)/5;
...
-(void)updateCurentStartTimePinPu
{
    ...
    for (int i = 0; i < self.scrollView.subviews.count; i++) {
        UIView *itemV = self.scrollView.subviews[i];
        if (itemV.tag >= 10000) {
            itemV.backgroundColor = [UIColor grayColor];
        }
    }
