...
@property(nonatomic,strong)DSUIButton * uploadBtn;//上传 按钮

@property(nonatomic,strong)DSUIButton * shootBtn;//拍摄 按钮
@property(nonatomic,strong)UIView * longPressView;//长按模式 长按区域
@property(nonatomic,strong)CAShapeLayer * shootAniLayer;

@property(nonatomic,strong)DSUIButton * delBtn;//删除当前拍摄内容
@property(nonatomic,strong)DSUIButton * finishedBtn;//完成拍摄按钮
...
{
    //拍摄按钮区域
    UIView * shootView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, (100), (100))];
    shootView.center = CGPointMake(self.frame.size.width/2, (50));
    [self addSubview:shootView];
    
    //拍摄按钮
    self.shootBtn = [DSUIButton buttonWithType:0];
    self.shootBtn.frame = CGRectMake(0, 0, (80), (80));
    self.shootBtn.center = CGPointMake((50), (50));
    self.shootBtn.layer.cornerRadius = (40);
    self.shootBtn.layer.masksToBounds = YES;
    self.shootBtn.backgroundColor = [UIColor systemPinkColor];
    ...
    //长按view
    self.longPressView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, (80), (80))];
    self.longPressView.center = CGPointMake((50), (50));
    self.longPressView.layer.cornerRadius = (40);
    ...
    _shootAniLayer.frame = CGRectMake(0, 0, (100), (100));
    _shootAniLayer.fillColor = [[UIColor clearColor] CGColor];
    _shootAniLayer.strokeColor = [[[UIColor systemPinkColor] colorWithAlphaComponent:0.5] CGColor];
    _shootAniLayer.opacity = 1;
    _shootAniLayer.lineCap = kCALineCapButt;
    _shootAniLayer.lineWidth = (8);
    _shootAniLayer.strokeEnd =  1.0;
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake((50), (50)) radius:(50) startAngle:-M_PI_2 endAngle:-M_PI_2+M_PI*2 clockwise:YES];
    _shootAniLayer.path =path.CGPath;
    [shootView.layer addSublayer:_shootAniLayer];
    
    //上传按钮
    self.uploadBtn = [DSUIButton buttonWithType:0];
    self.uploadBtn.imagePosition = DSUIButtonImagePositionTop;
    self.uploadBtn.frame = CGRectMake(0, 0, (50), (50));
    self.uploadBtn.center = CGPointMake(self.frame.size.width - (45) - (50)/2, (30) + (50)/2);
    ...
    //底部 拍摄类型bar
    self.typeBar = [[ShootTypeBar alloc] initWithFrame:CGRectMake(0, (100), self.frame.size.width, (50))];
    self.typeBar.delegate = self;
    [self addSubview:self.typeBar];
    
    //删除 完成按钮 默认隐藏
    self.delBtn = [DSUIButton buttonWithType:0];
    self.delBtn.frame = CGRectMake(0, 0, 40, 40);
    self.delBtn.center = CGPointMake(self.frame.size.width/2 + (50) + 20 + 20, (50));
    ...
    self.finishedBtn = [DSUIButton buttonWithType:0];
    self.finishedBtn.frame = CGRectMake(0, 0, 40, 40);
    self.finishedBtn.center = CGPointMake(self.frame.size.width/2 + (50) + 20 + 40 + 20 + 20, (50));
...
{
    //底部指示圆点
    self.bottomDian = [[UIView alloc] initWithFrame:CGRectMake(0, 0, (6), (6))];
    ...
    self.bottomDian.layer.cornerRadius = (6)/2;
    ...
    for (int i = 0; i < 3; i ++) {
        DSUIButton * btn = [DSUIButton buttonWithType:0];
...
-(void)updateSelectedIndex:(NSInteger)index
{
    ...
    for (int i = 0; i < 3; i ++) {
        DSUIButton * btn = (DSUIButton *)[self viewWithTag:i + 6600];
        if (i == index) {
...
-(void)itemClick:(DSUIButton *)sender
{
