...
@property(nonatomic,strong)DSUIButton * backFrontBtn;
@property(nonatomic,strong)DSUIButton * fastSlowBtn;
@property(nonatomic,strong)DSUIButton * beautifyBtn;
@property(nonatomic,strong)DSUIButton * timerBtn;
@property(nonatomic,strong)DSUIButton * crapMusicBtn;
@property(nonatomic,strong)DSUIButton * touchModeBtn;
...
-(void)updateVaptureDivicePosition:(AVCaptureDevicePosition)position
{
    ...
    if (position == AVCaptureDevicePositionBack) {
        [self.backFrontBtn setImage:AmShViImage(@"shoot_device_on") forState:0];
    }else{
        [self.backFrontBtn setImage:AmShViImage(@"shoot_device_off") forState:0];
    }
    [self updateItemsState];
}
...
- (void)updateTouchModeState:(AVCaptureTorchMode)torchMode
{
    ...
    if (torchMode == AVCaptureTorchModeAuto) {
        [self.touchModeBtn setImage:AmShViImage(@"shoot_flash_auto") forState:0];
    }else if (torchMode == AVCaptureTorchModeOn){
        [self.touchModeBtn setImage:AmShViImage(@"shoot_flash_on") forState:0];
    }else{
        [self.touchModeBtn setImage:AmShViImage(@"shoot_flash_off") forState:0];
    }
}
-(DSUIButton *)buildBtnTitle:(NSString *)title imgName:(NSString *)imgName index:(NSInteger)index
{
    ...
    [btn setImage:AmShViImage(imgName) forState:0];
    [self addSubview:btn];
