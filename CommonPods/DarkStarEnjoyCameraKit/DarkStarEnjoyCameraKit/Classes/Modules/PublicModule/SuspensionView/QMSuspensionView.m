...
- (void)toggleBelowInView:(UIView *)view touchView:(UIView *)touchView
{
    if (!self.superview) {
        [self showBelowInView:view touchView:touchView];
    }else {
        self.hidden = !self.hidden;
    }
}

- (void)showBelowInView:(UIView *)view touchView:(UIView *)touchView
{
    if (self.superview) {
        return;
    }
    CGRect tRect = [touchView convertRect:touchView.bounds toView:view];
    self.hidden = NO;
    
    CGFloat x = tRect.origin.x;
    CGFloat y = tRect.origin.y + tRect.size.height + kCameraRatioSuspensionViewMargin;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    imageView.image = [AECResouce imageNamed:model.icon];
