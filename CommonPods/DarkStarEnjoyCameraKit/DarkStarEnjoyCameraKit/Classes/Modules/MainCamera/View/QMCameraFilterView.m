...
+ (instancetype)cameraFilterView
{
    ...
    view.filterModels = [QMFilterModel buildFilterModelsWithPath:dirPath];
    return view;
}
...
{
    if (!self.slider) {
        self.slider = [[UISlider alloc] initWithFrame:CGRectMake(30, self.frame.origin.y-45, kScreenWidth-60, 30)];
        ...
        self.sliderLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.slider.frame.origin.x+self.slider.value*(kScreenWidth-90)-8, self.slider.frame.origin.y-30, 40, 30)];
        ...
        [[self.slider rac_signalForControlEvents:UIControlEventValueChanged] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            strongSelf.sliderLabel.frame = CGRectMake(strongSelf.slider.frame.origin.x+strongSelf.slider.value*(kScreenWidth-90)-8, strongSelf.slider.frame.origin.y-30, 40, 30);
        ...
        [[self.slider rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            __strong __typeof(weakSelf)strongSelf = weakSelf;
            QMFilterModel *model = strongSelf.filterModels[strongSelf.lastSelectedIndexPath.row];
            model.currentAlphaValue = strongSelf.slider.value;
            if (strongSelf.filterValueDidChangeBlock) {
                strongSelf.filterValueDidChangeBlock(model.currentAlphaValue);
            }
        }];
    }
...
- (void)showInView:(UIView *)view
{
    ...
    [UIView animateWithDuration:0.4f animations:^{
        ...
        [self->_collectionView scrollToItemAtIndexPath:self->_lastSelectedIndexPath atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:YES];
        [self selectConllectionViewAtIndex:self->_lastSelectedIndexPath];
    }];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!maskView) {
        ...
        imageView.image = [AECResouce imageNamed:@"qmkit_filter_adjust"];
