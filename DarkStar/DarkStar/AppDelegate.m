...
@interface AppDelegate ()<RERouterDelegate>
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [PhoneRouter singletonInstance].delegate = self;
...
- (void)changeWindowRootVC:(NSInteger)index {
    ...
    if ([REDataManagement isLogin]) {
        ...
        REUINavigationController *nav = [[REUINavigationController alloc] initWithRootViewController:loginVC];
        self.window.rootViewController = [REGlobal sharedInstance].globalNavC = nav;
    }
}
...
- (void)goToNoticeTabWithParams:(NSDictionary*)params {
    if (isStringEmptyOrNil([REGlobal sharedInstance].noticeTabDefaultSelectIndex)) {
        [REGlobal sharedInstance].noticeTabDefaultSelectIndex = [params getString:@"index"];
        [self goToTabWithIndex:[REGlobal sharedInstance].tab_noticeIndex];
    } else {
        [self postNotification:REReloadNoticeIndexNotification withObject:nil userInfo:params];
        [self goToTabWithIndex:[REGlobal sharedInstance].tab_noticeIndex];
    }
}


#pragma mark - RERouterDelegate
- (BOOL)checkMapVO:(RERouterVO *)vo
{
    if (vo.needLogin && ![REDataManagement isLogin]) {
        [[PhoneRouter singletonInstance] routerWithUrlString:[NSString getRouterVCUrlStringFromUrlString:kLogin andParams:nil] navigationController:[self ds_topViewController].navigationController];
        return NO;
    }
    return YES;
}
...
{
    if ([scheme isEqualToString:hjScheme] && [host isEqualToString:@"enterHome"]) {
        if ([self.window.rootViewController isKindOfClass:[REUITabBarController class]]) {
            [[REGlobal sharedInstance].globalTbc enterHomeWithIndex:@(0)];
        }
        return YES;
    }
    return NO;
